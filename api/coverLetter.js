...
module.exports = async (req, res) => {
	...
	const checkForSaved = async(url)=>{
		...
		const {data, error} = await supabase.from("li_bot").select("*").eq("linkedin_url", url)
		if(error){
			console.log(error)
...
if(!isSaved.status){
	...
	const saveCoverLetter = async(uuid, letter_data)=>{
		const {data, error} = await supabase.from("li_covers").insert({uuid, cover_letter:letter_data})
		if(error){
			console.log(error)
		}
		return
